...
get_file_date() {
    ...
    if command -v exiftool >/dev/null 2>&1; then
        ...
        if [[ -n "$date_found" && "$date_found" != "-" ]]; then
            # Convert from "2024:06:23 18:02:56" to "2024-06" format
            local year_month=$(echo "$date_found" | sed 's/:\([0-9][0-9]\):\([0-9][0-9]\).*/:\1/' | tr ':' '-')
...
main() {
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
        ...
        print_status $YELLOW "$0 \"$SOURCE_DIR\" \"$OUTPUT_DIR\" false"
    fi
}
